...
class TransformerPolicy:
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        # # add noise to different agents
        # if self.action_type == 'Discrete':
        #     self.num_actions_choice = round(self.act_dim / self.num_agents)
        #     self.agent_noise_list = []
        #     for i in range(self.num_agents):
        #         self.agent_noise_list.append(np.arange(
        #             i * self.num_actions_choice, (i + 1) * self.num_actions_choice
        #         ) if i < self.num_agents - 1 else np.arange(
        #             i * self.num_actions_choice, self.act_dim
        #         ))
        #     self.agent_noise_list = np.array(self.agent_noise_list)
        # else:
        #     self.agent_noise_list = None
    ...
    def act(self, cent_obs, obs, rnn_states_actor, masks, available_actions=None, deterministic=True):
        ...
        _, actions, _, rnn_states_actor, _ = self.get_actions(cent_obs,
                                                              ...
                                                              available_actions,
                                                              deterministic)
        # if self.args.add_agent_wise_noise and self.action_type == 'Discrete':
        #     print('actions', actions.shape)
        #     print('available_actions', available_actions.shape)
        #     random_actions = [
        #         np.random.randint(
        #             low=self.agent_noise_list[i][0], high=self.agent_noise_list[i][-1] + 1, size=(self.args.n_eval_rollout_threads, 1,)
        #         ) for i in range(self.num_agents)
        #     ]
        #     random_actions = torch.from_numpy(np.concatenate(random_actions, axis=1)).reshape(-1, 1)
        #     # print('random_actions', random_actions.shape)
        #     # print('random_actions', random_actions[2: 4, ])
        #     condition = torch.rand(*actions.shape) < self.args.agent_wise_noise_rate
        #     print('random_actions', available_actions[random_actions])
        #     print('random_actions', available_actions[random_actions].shape)
        #     actions = actions.detach().cpu()
        #     actions = torch.where(
        #         torch.rand(*actions.shape) < self.args.agent_wise_noise_rate,
        #         random_actions,
        #         actions,
        #     )
        #     print('actions', actions.shape)
    ...
    def save(self, save_dir, episode):
        if self.args.save_middle_model:
            ...
            torch.save(self.optimizer.state_dict(), str(save_dir) + "/optimizer.pt")

    def restore(self, model_dir, optim_dir):
        if model_dir is not None:
            transformer_state_dict = torch.load(model_dir) if torch.cuda.is_available() else torch.load(model_dir, map_location=torch.device('cpu'))
            self.transformer.load_state_dict(transformer_state_dict)
            print('--------------- load transformer -----------------')
        if optim_dir is not None:
            optimizer_state_dict = torch.load(optim_dir)
