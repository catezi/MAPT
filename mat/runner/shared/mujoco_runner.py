...
class MujocoRunner(Runner):
    ...
    def warmup(self):
        ...
        if self.all_args.use_preference_reward:
            self.pref_reward_assistant.his_next_obs[:, self.pref_reward_assistant.preference_traj_length - 1] = obs.copy()
    ...
    def eval(self, total_num_steps, faulty_node):
        ...
        while True:
            ...
            if eval_episode >= self.all_args.n_eval_rollout_threads:
    ...
    def sample_data(self):
        ...
        while True:
            ...
            eval_obs, eval_share_obs, eval_rewards, eval_dones, eval_infos, _ = self.eval_envs.step(eval_actions)
            eval_rewards1 = np.mean(eval_rewards, axis=1).flatten()
            one_episode_rewards += eval_rewards1
            ...
            # print('-----------------')
            # print('eval_now_obs', eval_now_obs.shape)
            # print('now_eval_share_obs', now_eval_share_obs.shape)
            # print('eval_actions', eval_actions.shape)
            # add data to data sampler
            if self.all_args.sample_icq_data:
                enough_data, total_steps_num = self.data_sampler.add_step_icq(
                    state=eval_now_obs,
                    share_state=now_eval_share_obs,
                    action=eval_actions,
                    reward=eval_rewards,
                    next_state=eval_obs,
                    next_share_state=eval_share_obs,
