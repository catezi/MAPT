...
from mat.runner.separated.base_runner import Runner
...
class HandsRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                obs, share_obs, rewards, dones, infos, _ = \
                ...
                if self.all_args.use_preference_reward:
                    data = obs, share_obs, pref_reward, dones, infos, \
    ...
    def warmup(self):
        ...
        for agent_id in range(self.num_agents):
            self.buffer[agent_id].share_obs[0] = _t2n(share_obs[:, agent_id]).copy()
            self.buffer[agent_id].obs[0] = _t2n(obs[:, agent_id]).copy()
    ...
    def insert(self, data):
        ...
        for agent_id in range(self.num_agents):
            ######################### add for preference reward (add dones info to data buffer)
            self.buffer[agent_id].insert(
                share_obs[:, agent_id], obs[:, agent_id], rnn_states[:, agent_id],
                rnn_states_critic[:, agent_id], actions[:, agent_id],
                action_log_probs[:, agent_id], values[:, agent_id], rewards[:, agent_id],
                dones[:, agent_id], masks[:, agent_id], None, active_masks[:, agent_id], None)
    ...
    def eval(self):
        ...
        while True:
            ...
            eval_obs, eval_share_obs, eval_rewards, eval_dones, _, _ = \
                self.eval_envs.step(torch.tensor(eval_actions))
            for eval_i in range(self.n_eval_rollout_threads):
    ...
    def preference_eval(self):
        ...
        while True:
            ...
            if self.all_args.use_preference_reward:
                ######################### preference: prefreence reward model
                pref_reward = self.pref_reward_assistant.insert(eval_obs, eval_actions, eval_dones)
                self.pref_reward_assistant.clear_done()
